...
用户存档保存在 `{config.data_path}/{config.diana_saves_dir}/{user_id}.json`，默认 `./data/asoul/diana/saves/`。YAML 配置和模板跟着包走（只读），saves 在外部数据目录（运行时可写）。
...
`from_dict()` 忽略不认识的键；新增字段对旧存档使用默认值。上线后如需不兼容的 schema 变更，再引入版本号和迁移链。